...
```
^([1-9][0-9]?|(?!127)1[0-9][0-9]?|2[0-2][0-3])(\.([0-9][0-9]?|1[0-9]{2}|2[0-4][0-9]|25[0-5])){3}$
```

### Match an IP Address from Class C only

```
^(19[2-9]|2[0-2][0-3])(\.([0-9][0-9]?|1[0-9]{2}|2[0-4][0-9]|25[0-5])){3}$
```
...
By default quantifiers are greedy, this means they will try to find the maximum amout of data to match from a string, this may not be useful as you may have repeated structures in the string where you want to capture just one instance of a repeated string. 
